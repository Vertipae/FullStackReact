...
const Country = ({ countries, filterHandler }) => {
...
// Jos yli tai yhtäkuin 10 maata pätee filtteröintiin niin kehoitetaan alla olevaa
    ...
    return (
        ...
        countries.map((country) => <div onClick={() => filterHandler(country.name)} value={country.name} key={country.name}>{country.name} </div>)
